...
class BreadcrumbsWidget extends \yii\base\Widget
{
    ...
    public function run() {
        ...
        return '<nav aria-label="Breadcrumb" class="mt-4">
    ...
    private function getBreadcrumbs($category_id)
    {
        ...
        $this->breadcrumbs = '
                ...
                <li><a href="'.Url::to(['catalog/index', 'alias'=>$category->alias]).'" class="block transition hover:text-sky-500">'.$category->title.'</a></li>'.$this->breadcrumbs;

        if(Category::find()->where(['id_import' => $category->parent_id])->count())
        {
